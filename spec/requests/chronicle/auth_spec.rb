...
RSpec.describe Chronicle::AuthController, type: :request do
  let!(:admin_user) { create(:chronicle_admin_user, email: 'bruno@example.com', password: 'password123') }
  let(:url) { '/chronicle/auth/login' }

  describe 'GET /chronicle/auth/login' do
    context 'when X-API-TOKEN is missing' do
      it 'returns 403' do
        get url
    ...
    context 'when X-API-TOKEN is wrong' do
      it 'returns 403' do
        get url, headers: { 'X-API-TOKEN' => 'wrong_token' }
        expect(response).to have_http_status(:forbidden)
      end
    end
